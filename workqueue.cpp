...
void* WorkQueue::worker_function(void *ptr) {
...
int WorkQueue::workqueue_init(workqueue_t *workqueue, int numWorkers) {
    int i;
	worker_t *worker;
...
void WorkQueue::workqueue_shutdown(workqueue_t *workqueue) {
...
void WorkQueue::workqueue_add_job(workqueue_t *workqueue, job_t *job) {
